...
trait LoopCommands {
  ...
  def hasFilesToLoad(settings: Settings): List[String] = settings match {
    case x: GenericRunnerSettings => x.loadfiles.value
    case _                        => Nil
  }

  // runs :load <file> on any files passed via -i
  def loadFiles(settings: Settings) = hasFilesToLoad(settings) foreach { filename =>
