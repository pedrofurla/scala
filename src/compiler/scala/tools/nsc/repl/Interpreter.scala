...
class Interpreter(val settings: REPLSettings)
            extends LowPriorityInterpreter
            with Binders
            with Classloading
            with Docs
            with Handlers
  ...
  def parseUnit(code: String): (CompilationUnit, Tree) = {
    val cunit   = new CompilationUnit(new BatchSourceFile("<console>", code))
    val scanner = new compiler.syntaxAnalyzer.UnitParser(cunit)
    val parsed  = scanner.compilationUnit()
    
    (cunit, parsed)
  }
  def parseComments(code: String) = parseUnit(code)._1.comments.toList
