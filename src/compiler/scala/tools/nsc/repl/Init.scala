...
trait Init {
  ...
  def enqueueReplAction(f: Interpreter => Unit) = replQueue put f
  
  /** Process any commands in the queue. */
  def processQueue()  = synchronized { while (!replQueue.isEmpty()) replQueue.take()(self) }
   ...
   *  until enqueueReplAction() is reached, at which point they're all run.
   */
  private val replQueue       = new LinkedBlockingQueue[Interpreter => Unit]
  ...
  class CompilerInitTask(callable: Callable[Global]) extends FutureTask(callable) {
    override protected def done() = {
      DBG("CompilerInitTask done, %d items in queue".format(replQueue.size))
