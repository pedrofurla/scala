...
object Spawn {
  ...
  def asyncIfPossible[T](action: () => T): SyncOrAsyncAction[T] = {
    def sync()      = new SyncInit(action)
    def async()     = new AsyncInit(() => new FutureTask[T](callableFn(action)))
    def tryAsync()  =
      try {
        val res = async()
        if (res.isStarted) Some(res) else None
      }
      catch {
        case _: Exception => None
      }
    
    tryAsync() getOrElse sync()
  }
