...
class InterpreterLoop(in0: Option[BufferedReader], protected val out: PrintWriter) extends LoopCommands {
  ...
  def printInteractive(msg: String) =
    if (isInteractive && hasFilesToLoad(settings).isEmpty)
      print(msg)
  ...
  protected def enqueueConvenienceVals() = {
    def enqueue(f: Interpreter => Unit): Unit = repl enqueueReplAction f
  ...
  protected def initializeInterpreter() = {
    if (isInteractive) {
      enqueueConvenienceVals()
      repl.initializeAsync()
    }
    else repl.initializeSync()
  }
  protected def createInitializedInterpreter() = {
    createInterpreter()
    repl.initializeSync()
  }
  ...
  def demandPatience() {
    printInteractive("Interpreter is still initializing, please wait...")

    // force the lazy val and then wait for the signal
    repl.compiler
    repl.initLatch.await()

    printInteractive("done.\n")
  }
  ...
  def command(line: String): Result = {
    ...
    // If there are queued commands they take precedence
    repl.processQueue()
  ...
  def main(settings: Settings) {
    ...
    val toLoad = hasFilesToLoad(settings)
    
    if (toLoad.nonEmpty) {
      plushln("Initializing repl: " + toLoad.size + " file" + plural(toLoad) + " to preload.")
      createInitializedInterpreter()
    }
    else createInterpreter()
